...
namespace VuongLeTrung.Exercise02.Controllers
{
    ...
    public class GalleryController : ControllerBase
    {
        ...
        [HttpPut("multiple/{productId}")]
        public IActionResult PutMultipleImages(Guid productId, [FromForm] List<IFormFile> imageFiles)
        {
            try
            {
                //// Xóa tất cả các ảnh cũ có cùng ID sản phẩm
                //var existingImages = _db.Galleries.Where(g => g.ProductId == productId);
                //_db.Galleries.RemoveRange(existingImages);
