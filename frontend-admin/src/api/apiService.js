...
export function GET_ALL_TAGS(endpoint) {
    return callApi(endpoint, "GET");
}
...
export async function POST_ADD_CATEGORY(formData) {
    try {
        const response = await axios.post("http://localhost:5284/api/Categories", formData);
...
export async function PUT_EDIT_CATEGORY(id, formData) {
    try {
        const response = await axios.put(`http://localhost:5284/api/Categories/${id}`, formData);
        return response.data;
    } catch (error) {
        throw error;
    }
